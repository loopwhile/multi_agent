...
2. `@docs/03_rules.md`
3. 현재 담당할 역할 `@ai_prompts/roles/{역할}.md`
...
- 1회의 프롬프팅으로 너무 많은 단계(기획→설계→구현)를 건너뛰려 하지 말고, `ai_prompts/workflows`에 정의된 Gate 단위로 멈추고 승인을 요청하라.

## 4. Work Order 생성 & 진행 방식
- 사용자가 "WO 하나 만들어줘"라고 지시하면 `@ai_prompts/tasks/_template.md`를 참고하여 채팅창에 작성해 주거나 지정된 파일에 저장한다.
