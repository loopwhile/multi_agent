...
adapter spec 원문은 `adapters/_adapter_spec.md`에 있다.
...
```
운영 규약의 원천 = repo 안의 공용 문서 (`docs/`, `roles/`, `workflows/`, `validation/`, `tasks/`, `adapters/`)
...
| 금지 항목 | 이유 | 올바른 위치 |
|-----------|------|-------------|
| 역할의 책임 정의 | 역할 분산 → 불일치 발생 | `roles/` |
| 워크플로우 본문 | 도구별 워크플로우 분기 금지 | `workflows/` |
| 검증 기준 | 도구별 품질 기준 차등 금지 | `validation/` |
| Output Contract | 출력 형식의 도구별 변형 금지 | `validation/output_contract.md` |
...
1. `adapters/_adapter_spec.md`의 공통 구조를 따른다
...
- adapter spec: `adapters/_adapter_spec.md`
- 기존 adapter 예시: `adapters/antigravity.md` 외 3개
