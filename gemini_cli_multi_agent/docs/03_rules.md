...
> 운영 규약의 원천은 `ai_prompts/`의 공용 문서다.  
> 특정 도구에서만 동작하는 규칙을 공용 코어에 넣지 않는다.  
> 도구별 차이는 `ai_prompts/adapters/`에서만 처리한다.
...
모든 에이전트의 출력은 `ai_prompts/validation/output_contract.md`의 형식을 따른다.
...
공용 코어 문서(`ai_prompts/` 또는 `docs/`)를 수정할 때는  
